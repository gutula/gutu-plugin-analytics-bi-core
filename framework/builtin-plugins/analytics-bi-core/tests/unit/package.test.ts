...
import { businessFlowDefinitions, publishAnalyticsDataset, refreshKpiDefinitions, enqueueWarehouseSync, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("analytics-bi-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("analytics.datasets.publish");
    expect(manifest.publicEvents).toContain("analytics.dataset-published.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof publishAnalyticsDataset).toBe("function");
    expect(typeof refreshKpiDefinitions).toBe("function");
    expect(typeof enqueueWarehouseSync).toBe("function");
