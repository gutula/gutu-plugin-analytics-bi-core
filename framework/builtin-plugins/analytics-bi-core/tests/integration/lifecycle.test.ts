...
describe("analytics-bi-core lifecycle integration", () => {
  const previousStateDir = process.env.GUTU_STATE_DIR;
  let stateDir = "";

  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "analytics-bi-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "analytics-bi-core:demo",
      title: "Analytics & BI Core Demo",
      ...
      correlationId: "analytics-bi-core:corr",
      processId: "analytics-bi-lifecycle:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "analytics-bi-core:demo",
      exceptionId: "analytics-bi-core:exception",
    ...
    const amended = await amendPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "analytics-bi-core:demo",
      amendedRecordId: "analytics-bi-core:demo:amended",
      expectedRevisionNo: 5,
      title: "Analytics & BI Core Demo Amendment",
      reasonCode: "commercial-correction"
    });
    expect(amended.amendedRecordId).toBe("analytics-bi-core:demo:amended");

    const reversed = await reversePrimaryRecord({
      tenantId: "tenant_demo",
      actorId: "actor_admin",
      recordId: "analytics-bi-core:demo:amended",
      reversalRecordId: "analytics-bi-core:demo:amended:reversal",
      expectedRevisionNo: 1,
      reasonCode: "cancel-amendment"
    });
    expect(reversed.reversalRecordId).toBe("analytics-bi-core:demo:amended:reversal");
