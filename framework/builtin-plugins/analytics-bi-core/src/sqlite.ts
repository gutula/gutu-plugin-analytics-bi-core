export type AnalyticsBiCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildAnalyticsBiCoreSqliteMigrationSql(options: AnalyticsBiCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "analytics_bi_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getAnalyticsBiCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getAnalyticsBiCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildAnalyticsBiCoreSqliteRollbackSql(options: AnalyticsBiCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "analytics_bi_core_");
  return [
...
export function getAnalyticsBiCoreSqliteLookupIndexName(tablePrefix = "analytics_bi_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getAnalyticsBiCoreSqliteStatusIndexName(tablePrefix = "analytics_bi_core_"): string {
