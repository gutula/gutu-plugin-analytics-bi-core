export type AnalyticsBiCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildAnalyticsBiCoreMigrationSql(options: AnalyticsBiCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "analytics_bi_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getAnalyticsBiCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getAnalyticsBiCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildAnalyticsBiCoreRollbackSql(options: AnalyticsBiCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "analytics_bi_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "analytics_bi_core";
  return [
...
export function getAnalyticsBiCoreLookupIndexName(): string {
  return "analytics_bi_core_primary_lookup_idx";
}

export function getAnalyticsBiCoreStatusIndexName(): string {
  return "analytics_bi_core_exception_status_idx";
}
